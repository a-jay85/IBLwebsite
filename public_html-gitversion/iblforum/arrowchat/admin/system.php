	...
	if (var_check('config_submit')) 
	{
		...
		$result = $db->execute("
			...
			SET config_value = CASE 
				...
				WHEN config_name = 'push_secret' THEN '" . get_var('push_secret') . "'
			END WHERE config_name IN ('channel_name', 'base_url', 'online_timeout', 'disable_smilies', 'heart_beat', 'buddy_list_heart_beat', 'idle_time', 'push_on', 'push_publish', 'push_subscribe', 'push_secret')
		");
		...
		if ($result && $heart_beat_test) 
		{
			...
			$push_secret = get_var('push_secret');
