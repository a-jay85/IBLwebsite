...
define('FILE_VERSION', '4.1.3'); // this should match installsteps.php
define('SIMPLE_VERSION', '413'); // see vB_Datastore::check_options()
...
*
* @package	vBulletin
* @version	$Revision: 42496 $
* @date		$Date: 2011-03-31 18:20:00 -0700 (Thu, 31 Mar 2011) $
*/
class vB_Database
{
	/**
	...
	function clean_array_gpc($source, $variables)
	{
		...
		foreach ($variables AS $varname => $vartype)
		{
			...
			if (!isset($this->cleaned_vars["$varname"]) OR $this->cleaned_vars["$varname"] != $vartype)
			{
				...
				if ((defined('VB_API') AND VB_API === true) OR $GLOBALS[$this->superglobal_lookup['r']]['forcenoajax'])
				{
					switch ($vartype) {
						case TYPE_STR:
						case TYPE_NOTRIM:
